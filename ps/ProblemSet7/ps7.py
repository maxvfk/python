...
class RectangularRoom(object):
    ...
    def __init__(self, width, height):
        ...
        self.width=width
        self.height=height
        self.field={(x,y):0 for x in range(width) for y in range(height)}
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        self.field[int(pos.getX()),int(pos.getY())]=1;
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        npos=self.pos.getNewPosition(self.dir,self.speed)
        if self.room.isPositionInRoom(npos):
            self.pos=npos
            self.room.cleanTileAtPosition(npos)
        else:
            self.dir=random.randint(0,359)
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        robolist=[robot_type(room,speed) for n in range(num_robots)]
        while room.getNumCleanedTiles()<min_tiles:
            #anim.update(room, robolist)
            for robo in robolist:
                robo.updatePositionAndClean()
            trials+=1
        trlist.append(trials)
        #anim.done()
    return sum(trlist)/len(trlist)
